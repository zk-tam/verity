...
export function EpicChaseCardsCarousel({
  cards,
  epicMetadata,
}: RecentPullsCarouselProps) {
  ...
  const spacerWidth = Math.max(
    8,
    Math.round(viewportWidth / 2 - (isCompact ? 120 : isTablet ? 260 : 400))
  );
  ...
  const selectedPull = centerIndex >= 3 ? cards[centerIndex - 3] : null;
  ...
  return (
    <GradientLiquidGlass
      ...
      rounded="2xl"
      style={{
        boxShadow: "inset 0 0 5px 0 rgba(0, 0, 0, 1)",
    ...
    >
      ...
      <div className="text-center space-y-2">
        ...
        >
          {epicMetadata.tierName ?? "Epic"}
        </h2>
        <div
          className="text-white font-bold"
          style={{ textShadow: "0 0 2px #ffffff81" }}
        >
          {epicMetadata.priceRange}
        </div>
      </div>

      <div className="absolute top-3 right-3">
        <div className="text-white text-lg font-bold bg-black/30 rounded-full px-4 py-1">
          {epicMetadata.oddsPct}%
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <button
          onClick={() =>
            scrollToIndex(Math.min(cards.length + 2, centerIndex + 1))
          }
          ...
          disabled={centerIndex === cards.length + 2}
        >
          ...
          <div className="flex items-center py-8 relative">
            ...
            {cards.map((card, index) => {
              ...
              if (adjustedIndex === 3) {
                ...
                else if (minScale >= 0.7)
                  marginLeft = overlapMedium; // Medium cards
                else marginLeft = overlapSmall; // Small cards
              }

              const gradeLabel = card.grading_company
      ...
      {selectedPull && (
        <div className="w-full mx-auto p-4">
          <div className="text-center space-y-2 text-white font-loose-">
            <h3 className="text-lg font-loos-extra-wide-medium uppercase">
              {targetCardName}
            </h3>
